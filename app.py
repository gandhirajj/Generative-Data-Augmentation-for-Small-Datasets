...
def synthesize_tabular(df: pd.DataFrame, n_samples: int, target: Optional[str]=None) -> Tuple[pd.DataFrame, str]:
    ...
    # Heuristic: if categorical columns are almost all unique, CTGAN will stall
    cat_cols = df.select_dtypes(exclude=[np.number]).columns
    if any(df[c].nunique() > 0.9 * len(df) for c in cat_cols):
        real = df.copy()
        synth_rows = real.sample(n_samples, replace=True).reset_index(drop=True)
        return synth_rows, "Bootstrap (high-cardinality fallback)"
...
def synonym_augment_wordnet(text: str, n_swaps: int = 2) -> str:
    ...
    for i in idxs:
        ...
        if lemmas:
            tokens[i] = random.choice(lemmas); swaps += 1
...
def augment_image_pil(img: Image.Image, n: int = 5) -> List[Image.Image]:
    ...
    out = []
    for _ in range(n): out.append(pipe(img))
    return out
...
def generate_with_sd(prompt: str, n: int = 4, guidance_scale: float = 7.5, steps: int = 30):
    try:
        ...
        warn(f"Stable Diffusion not available or failed: {e}"); return []
...
if section == "Tabular":
    ...
    if file is not None:
        ...
        if st.button("Generate Synthetic"):
            ...
            if "Bootstrap" in method:
                warn(f"Used fallback method: {method}")
            else:
    ...
    if st.button("Generate Paraphrases"):
        ...
        with st.spinner("Generating..."): aug = augment_texts(lines, k=k)
        for src, variants in aug.items():
            st.markdown(f"**Source:** {src}")
            for j, v in enumerate(variants, 1): st.write(f"{j}. {v}")
            st.divider()
        rows = [{"source": src, "augmented": v} for src, variants in aug.items() for v in variants]
        df_out = pd.DataFrame(rows); buf = io.BytesIO(); df_out.to_csv(buf, index=False)
    ...
    if st.button("Run Image Augmentation"):
        ...
        with zipfile.ZipFile(zip_buf, mode='w', compression=zipfile.ZIP_DEFLATED) as zf:
            for f in imgs or []:
                ...
                except Exception as e: warn(f"Failed to open {f.name}: {e}"); continue
                ...
                for i, im in enumerate(augd):
                    ...
                    if i < len(prev_cols): prev_cols[i].image(im, caption=out_name.split('/')[-1], use_column_width=True)
